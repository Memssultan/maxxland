    ...
    >
      <Card className="h-full transition-all duration-300 hover:shadow-lg">
        <CardContent className="flex flex-col items-center text-center p-3 sm:p-4 space-y-2 sm:space-y-3 h-full">
          ...
          <h3 className="text-sm sm:text-base text-red-700 font-baskerville-bold">{title}</h3>
          <p className="text-xs sm:text-sm text-muted-foreground flex-grow font-baskerville-italic">{description}</p>
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" className="text-red-700 mt-1 sm:mt-2 text-xs sm:text-sm px-2 py-1 h-auto font-baskerville-italic">
                Подробнее <ChevronRight className="ml-1 h-3 w-3" />
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle className="font-baskerville-italic">{title}</DialogTitle>
              </DialogHeader>
              <p className="text-sm font-baskerville-italic">{details}</p>
            </DialogContent>
          </Dialog>
        </CardContent>
      </Card>
  ...
  return (
    <section className="py-6 sm:py-8 md:py-12 px-2 sm:px-4 bg-gradient-to-b from-background to-primary/5">
      <div className="container mx-auto">
        ...
        >
          <h2 className="text-xl sm:text-2xl md:text-3xl text-center text-primary mb-2 sm:mb-3 font-baskerville-bold">Наши преимущества</h2>
